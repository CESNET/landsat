...
class M2M(object):
    ...
    def searchScenes(self, datasetName, **args):
        ...
        if scenes['totalHits'] > scenes['recordsReturned']:
            logging.warning(
                'M2M.searchScenes - more hits {} than returned records {}, consider increasing maxResults parameter.'.format(
                    scenes['totalHits'],
    ...
    def retrieveScenes(self, datasetName, scenes, filterOptions={}, label='m2m-api_download'):
        ...
        labels = [label]
        downloadOptions = self.downloadOptions(datasetName, filterOptions, listId=label,
        ...
        if requestedDownloadsCount:
            ...
            if requestResults['preparingDownloads'] != None and len(requestResults['preparingDownloads']) > 0:
                ...
                while len(downloadIds) < requestedDownloadsCount:
                    ...
                    logging.info('M2M.retrieveScenes - {} downloads are not available. Waiting 10 seconds...'.format(
                        preparingDownloads))
                    time.sleep(10)
                    for label in labels:
                        requestResultsUpdated = self.downloadRetrieve(label)
